    ...
    <div class="container mt-5">
        ...
        <form action="{{ route('customers.update', $customer->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control" id="total_stay_cost" name="total_stay_cost" value="{{ old('total_stay_cost', $customer->total_stay_cost) }}" required min="0">
                @error('total_stay_cost')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control" id="down_payment" name="down_payment" value="{{ old('down_payment', $customer->down_payment) }}" required min="0">
                @error('down_payment')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
...
{{-- <div class="mb-3">
    <label class="form-label">Spese aggiuntive</label>
    <div id="additional-expenses-container">
        @php
            $additionalExpenses = json_decode($customer->additional_expenses, true) ?? [];
        @endphp
        @foreach($additionalExpenses as $expense => $amount)
        <div class="input-group mb-2">
            <span class="input-group-text expense-label">{{ ucfirst($expense) }}</span>
            <input type="number" step="0.01" class="form-control expense-amount" name="additional_expenses[{{ $expense }}]" value="{{ old('additional_expenses.' . $expense, $amount) }}" required>
            <button type="button" class="btn btn-danger remove-expense-btn">Rimuovi</button>
        </div>
        @endforeach
    </div>
    <select class="form-control mb-2" id="add-expense-select">
        <option value="">Aggiungi una spesa...</option>
