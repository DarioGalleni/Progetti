...
$firstDayOfCalendar = $selectedMonth
    ->copy()
    ->startOfMonth()
    ->startOfWeek(Carbon::MONDAY);

$lastDayOfCalendar  = $selectedMonth
    ->copy()
    ->endOfMonth()
...
<x-layout>
    ...
    <div class="container-fluid mt-4">
        ...
        <div class="d-flex justify-content-between mb-3">
            <a
                href="{{ route('welcome', ['month' => $previousMonth->format('Y-m')]) }}"
                class="btn btn-secondary"
            >
                ← Mese Precedente
            </a>
            <a
                href="{{ route('welcome', ['month' => Carbon::now()->format('Y-m')]) }}"
                class="btn btn-info"
            >
                Mese Corrente
            </a>
            <a
                href="{{ route('welcome', ['month' => $nextMonth->format('Y-m')]) }}"
                class="btn btn-secondary"
            >
                Mese Successivo →
            </a>
        ...
        >
            ...
            >
                <thead class="table-light sticky-top">
                    <tr>
                        <th
                            class="sticky-header-room bg-light"
                ...
                <tbody>
                    @php
                        ...
                        // Etichette prese da merged.jpg (chiave = numero camera, valore = descrizione)
                        $roomLabels = [
                            102 => 'TR V.P',
                            103 => 'TR V. P',
                            104 => 'QD V.P',
                    ...
                    @foreach($roomLabels as $roomNumber => $roomLabel)
                        <tr>
                            <td
                                class="sticky-header-room bg-light"
                                style="position: sticky; left: 0; z-index: 1; min-width: 140px;"
                            >
                                <div class="d-flex flex-column align-items-start">
                                    <strong style="font-size:1.25rem;">{{ $roomNumber }}</strong>
                                    <span style="font-size:0.9rem;">{{ $roomLabel }}</span>
                                </div>
                            </td>
                            ...
                            @while($currentDate->lte($lastDayOfCalendar))
                                ...
                                @else
                                    @php
                                        $foundBooking = collect($roomBookings[$roomNumber] ?? [])
                                            ->first(function($booking) use ($currentDate) {
                                    ...
                                    @if($foundBooking)
                                        ...
                                        >
                                            <a
                                                href="{{ route('customers.show', $foundBooking->id) }}"
                                            ...
                                            >
                                                ...
                                                >
                                                    <div>
                                                        {{ $foundBooking->first_name }}
                                                        {{ $foundBooking->last_name }}
                                                    </div>
                                                    <small>
                                                        {{ $foundBooking->number_of_people }}
                                                        pers. – {{ $foundBooking->treatment }}
                                                    </small>
                                                </div>
                                            </a>
                                        </td>
